"""
https://github.com/NewsJelly/semantle-ko
"""
...
if __name__ == '__main__':
    ...
    with open('data/frequent_words.txt', 'r', encoding='UTF-8') as f:
        words = list(unicodedata.normalize('NFC', line.strip())
    ...
    for index, out in enumerate(tqdm.tqdm(pipe(x for x in words), total=len(words))):
        ...
        try:
            if label['label'] == 'clean':
                ...
                print(
                    f'filtered: {words[index]} - {label["label"]}/{label["score"]}')
        except:
    ...
    with open('data/ko-aff-dic-0.7.92/ko.dic', 'r', encoding='UTF-8') as f:
        words = list(unicodedata.normalize(
            'NFC', line.strip().split('/')[0]) for line in tqdm.tqdm(f))
    filtered_words = []
